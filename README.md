...
- **Веб-интерфейса** для удобного управления через браузер
...
Модуль включает современный веб-интерфейс для управления проверками здоровья через браузер.

### Установка веб-интерфейса

1. **Скопируйте файл веб-интерфейса** в доступное для Nginx место:
   ```bash
   cp web-ui.html /usr/share/nginx/html/healthcheck-ui.html
   ```

2. **Настройте Nginx** для обслуживания веб-интерфейса и API endpoints:
   ```nginx
   server {
       listen 8888;
       
       # Веб-интерфейс
       location = /healthcheck-ui.html {
...
3. **Откройте в браузере**:
   ```
   http://localhost:8888/healthcheck-ui.html
   ```

Веб-интерфейс автоматически определит базовый URL API из текущего местоположения страницы. Вы также можете вручную указать его в поле ввода вверху.

### Возможности веб-интерфейса

- **Обзор upstream**: Просмотр всех настроенных upstream блоков с их типом (HTTP/HTTPS/TCP/SSL) и статусом
- **Отображение конфигурации**: Просмотр текущих параметров проверки здоровья (fall, rise, interval, timeout и т.д.)
- **Статус серверов**: Мониторинг статуса отдельных серверов с счетчиками fall/rise
- **Быстрые действия**: Включение/отключение upstream или отдельных хостов одним кликом
- **Редактор конфигурации**: Обновление параметров проверки здоровья через удобную форму
- **Фильтр**: Фильтрация upstream по имени для удобной навигации
